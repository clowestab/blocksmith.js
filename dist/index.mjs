...
function to_address(x) {
	if (x) {
		...
		if (is_address(x.contractAddress)) return x.address;
	}
}
	...
	async deploy(arg0) {
		...
		if (!this.isAnvil()) {
			...
			if (deployment && 'target' in deployment) {
				const deployedContract = new ethers.Contract(deployment.target, deployment.abi.fragments, from);
...
export { Foundry, FoundryBase, Node, Resolver, compile, error_with, execCmd, is_address, mergeABI, to_address };
